...
**Status:** Current
**Last modified:** 2026-06-15 12:23 EDT
...
> **Byte-parity authority.** This page counts *flag-implementation* progress
> (does a flag have a consuming field, does it behave on the common case). It
> is not the byte-for-byte parity signal; "Done" here means implemented and
> accepted, not output-byte-matched against legacy CLAN. The honest
> byte-parity status, including the generated golden-coverage table, lives in
> [Parity Status](../parity-status.md), and per that page no command yet
> achieves byte-for-byte parity.
...
|---|---|---|---|
| **Done** | 220 | 33% | Implemented and accepted (not necessarily byte-parity) |
...
Tracking what just landed so future sessions have context. Before this repo
became standalone, each item was typically a single commit on `main` in the
earlier monorepo.
