...
// See analysis::dispatch for the result_large_err rationale (Err is the
// unhandled clap command, pattern is control flow not error handling).
...
pub(super) fn dispatch(command: ClanCommands) -> Result<(), ClanCommands> {
    match command {
        ClanCommands::Check {
            ...
            // Fail closed on flag-shaped tokens no command consumed.
        ...
        ClanCommands::Gemfreq { path, mor, common } => {
            run_analysis_and_print(
                AnalysisOptions::Freq(talkbank_clan::service_types::FreqOptions {
                    count_source: if mor {
                        talkbank_clan::commands::freq::CountSource::MorStructural
                    } else {
                        talkbank_clan::commands::freq::CountSource::MainTier
                    },
                    ..Default::default()
                }),
                &path,
