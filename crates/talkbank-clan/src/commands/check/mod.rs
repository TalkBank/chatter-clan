...
pub struct CheckResult {
    /// Path to the file that was checked. A path type, not a `String`: it is
    /// rendered to a forward-slash string only at the output boundary
    /// (`check_path_display`).
    pub file: PathBuf,
...
/// Render a checked-file path for CHECK output. This is the single place a
/// [`Path`] becomes a string in CHECK output, and it normalizes to forward
/// slashes on every platform: CHECK output is user-facing and snapshot-compared,
/// and CLAN and the CHAT convention use `/`, so a Windows `\` separator must not
/// leak in. No-op on Unix.
fn check_path_display(path: &Path) -> String {
