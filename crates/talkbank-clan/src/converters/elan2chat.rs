...
// `normalized_value(version)`. The replacement is not a drop-in: it
// requires an `XmlVersion` argument selected per-attribute. ELAN files
// declare XML 1.0 in the header, but threading the version through every
// attribute read is non-trivial without restructuring this converter.
// Suppress for now; revisit when the converter is rewritten or quick-xml
