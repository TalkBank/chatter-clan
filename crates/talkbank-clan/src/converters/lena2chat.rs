// quick-xml 0.40 deprecated `Attribute::unescape_value()`; see the
// matching note in `elan2chat.rs` for why we keep the deprecated path
// rather than migrate to `normalized_value(XmlVersion)`.
#![allow(deprecated)]
