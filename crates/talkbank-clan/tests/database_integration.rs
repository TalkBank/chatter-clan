...
/// Resolve CLAN library path: CLAN_SOURCE_DIR env → sibling OSX-CLAN/ → ~/OSX-CLAN/
fn clan_lib_path(subpath: &str) -> String {
    ...
    // Runtime `HOME` lookup so this file compiles on Windows (no `HOME`
    // env var), the empty-string fallback yields a path like
    // `/OSX-CLAN/...` which the caller's `skip_if_missing` then skips.
    let home = std::env::var("HOME").unwrap_or_default();
    format!("{home}/OSX-CLAN/{subpath}")
}
